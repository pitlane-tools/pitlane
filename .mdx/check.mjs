...
//
//   node .mdx/check.mjs [tsconfig.json ...]     default: docs/tsconfig.json
//
...
function readProject(tsconfigPath) {
...
function startServer() {
    ...
    let serverVersion = JSON.parse(ts.sys.readFile(path.join(serverBin, "../../package.json"))).version;
    ...
    let closed = new Promise((_, reject) => {
        connection.onClose(() => {
            closedEarly = true;
...
async function diagnose(server, fileName, expectedTsconfig) {
...
function location(fileName, { start }) {
    return `${display(fileName)}:${start.line + 1}:${start.character + 1}`;
}

function formatDiagnostic(fileName, diagnostic) {
    let severity = SEVERITY_LABELS[diagnostic.severity] ?? "error";
    let code =
        diagnostic.source === "ts"
            ? `TS${diagnostic.code}`
            : `${diagnostic.source} ${diagnostic.code ?? ""}`.trim();
    let lines = [`${location(fileName, diagnostic.range)} - ${severity} ${code}: ${diagnostic.message}`];
...
async function main(args) {
    let projects = (args.length > 0 ? args : [path.join(REPO_ROOT, "docs/tsconfig.json")]).map(readProject);
    ...
    let plural = (count, noun) => `${count} ${noun}${count === 1 ? "" : "s"}`;
    console.log(
