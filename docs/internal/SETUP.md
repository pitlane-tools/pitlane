...
CSS, scripts, search indexes, and Markdown exports are static assets. Cookie-dependent document responses use private, non-shared caching. A reader's old localStorage preference can only migrate after JavaScript runs; the first server response cannot read browser storage.
