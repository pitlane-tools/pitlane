...
import { Install } from "../../.docs/app/components/documentation.tsx";
...
<Install packages={["@pitlane/theme"]} />
...
Inline `css()` infers the element type from `mix`. For fixed styles reused on one element type, create a descriptor once with an explicit type, such as `css<HTMLAnchorElement>({ color: t.color.page })`. Share `ThemedCSSProps` objects instead when the same styles are used on different element types.
