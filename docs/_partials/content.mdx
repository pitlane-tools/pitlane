import { Callout, InstallGroup, NoBuild, Vite } from "../../.docs/app/components/documentation.tsx";
import { installAlternatives } from "../../.docs/app/install.ts";

export const viteInstallation = await installAlternatives({
    packages: ["@pitlane/content"],
    dev: ["satteri", "vite-plugin-satteri"],
});

export const noBuildInstallation = await installAlternatives({
    packages: ["@pitlane/content", "satteri"],
});
...
<InstallGroup alternatives={viteInstallation} />
...
<InstallGroup alternatives={noBuildInstallation} />
...
<Callout kind="warning" title={<>MDX needs a host that allows <code>new Function</code></>}>
...
<Callout kind="warning" title="A new content file needs a restart">
