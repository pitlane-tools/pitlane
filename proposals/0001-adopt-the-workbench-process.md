...
- [x] Every gate `mise run check` runs passes over the files this change touches: `docs:build`, `validate`, `tools:test`, `tsc`, and `oxfmt --check` and `oxlint` over `tools/`, `.omp/`, `.agents/`, and the record. `mise run check` as a whole does not pass in the working tree it landed in, for two reasons that predate it and live in files it never touched: `oxlint` reports errors in `demos/theme/remix.plugin.ts` and `docs/.vitepress/theme/**`, and `oxfmt --check` flags `pnpm-workspace.yaml`.
