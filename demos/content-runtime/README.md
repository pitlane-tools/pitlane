...
fingerprints, and no watcher, channel, or instrumentation is constructed.

`dev` puts `hmr.ts` in front of it. That process supervises the server, applies
an accepted module change in place, restarts it when a change is not accepted,
and holds requests until the new generation answers. `remix/ui-hmr/node` and
the `uiHmr()` asset loader are what let a component update on both sides
without losing its state — edit `app/ui/public/counter.tsx` while the counter
reads `clicked 3 times` and it still reads 3 afterwards. Editing a content file
is the exception: nothing imports it, so it takes a restart.
