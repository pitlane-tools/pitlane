...
State your recommendation, then ask whether the human wants a proposal. Always wait for explicit approval before creating one or opening a proposal-only branch or pull request. Ask even when the human said "start a feature" or "write a proposal" — only their answer to this question, for this change, is consent. A request to implement something, a selected tool, answers to design questions, urgency, or your own assessment that work is substantial do not grant permission. Announcing that you will write a proposal is not asking.
...
After the human approves creating a proposal, confirm the change has a branch pushed to `origin` and an open draft pull request before writing it. The pull request is the durable workspace, and a proposal iterated anywhere else is a proposal the human cannot comment on. That approval covers this proposal and its later revisions; a different change needs its own answer.
...
- Complete every template section: Summary, Motivation, Domain grounding, Existing baseline, Proposed solution, Detailed design, Compatibility, Implications on adoption, Scope, Out of scope, Preview, Policies and decisions checked, Future directions, Alternatives considered, and Open questions. Acknowledgments is optional.
...
2. **Ground the problem.** When the change depends on domain knowledge that cannot safely be inferred from the request, use [`.agents/skills/grounding-a-problem/`](../grounding-a-problem/) before comparing solutions. Prefer primary or authoritative evidence, and keep it separate from repository observations, inference, and hypotheses. In **Domain grounding**, record the concepts, constraints, quality bar, and remaining uncertainty a reader needs to judge the proposal. When no independent grounding is needed, say why there rather than manufacturing research.

3. **Review the ontology when the change is concept-heavy.** If it introduces or changes important concepts, identity, relationships, states, classifications, or vocabulary, use [`.agents/skills/ontology-review/`](../ontology-review/) against the grounded domain before the design settles. Resolve material conflicts in the proposal or back in grounding; never encode them silently.

4. **Study the baseline.** After grounding, inspect the closest existing implementation, prototype, workflow, or earlier attempt — a package under `packages/`, a demo, or prior art outside this repository. In **Existing baseline**, name what it gets right and what is incidental or compromised. Existing behavior is evidence of what happens today, not authority about the domain. When no relevant baseline exists, say so instead of inventing one.

5. **Discover one answer at a time.** Ask one question, wait for its answer, then ask the next. Use multiple-choice questions when the answer space is finite. Cover:
...
6. **Compare approaches.** Present two or three viable approaches with tradeoffs. Recommend one and explain why. Ask the human to select an approach or direct a revision before treating the design as settled.

7. **Choose the preview now.** Select which of pitlane's two preview mechanisms exercises the changed behavior. Record it as the artifact in **Preview**, with the reason it is the cheapest realistic exercise of this change.
...
8. **Author the proposal.** Fill the template from the grounding, baseline, answers, and selected approach. State behavior and boundaries plainly. Record rejected approaches and why they lost. Keep unresolved questions as markers rather than inventing answers.

9. **File unrelated defects.** If discovery reveals a defect outside this proposal's scope, file it as a GitHub issue. Discovering a defect is never a reason to expand the proposal or forget it. Use [`.agents/skills/tracking-defects/`](../tracking-defects/) for the judgment and filing process.

10. **Self-review.** Before showing the human, check:
    - every template section and frontmatter field is complete;
    - the grounding, baseline, motivation, behavior, criteria, scope, preview, and record check agree;
...
11. **Publish the draft.** Commit the proposal and push it as soon as it is coherent enough to react to. Do not polish it privately first — the draft pull request exists so the human can read and edit the proposal in place, and a proposal held back is a proposal being decided by the agent alone.

12. **Iterate on the pull request.** The human comments, requests changes, and may commit edits to the proposal directly. Read those commits before responding; they are instructions, not conflicts to resolve. Keep iterating — answering questions, revising the design, resolving markers — until the human is satisfied. This can take many rounds, and the proposal stays `draft` throughout.

13. **Wait for the human to close the draft.** The human decides when the proposal is settled and moves it to `status: awaiting-implementation`. They may ask you to make that edit; making it when asked is fine, deciding it is not.
...
| --- | --- |
| No explicit approval to create a proposal for this change | Ask whether the human wants one, then wait — before the branch, the draft pull request, or the file. |
...
| Preview names no exercisable artifact for a behavior change | Choose and record one before approval. |
...
**Do not use the existing implementation to teach yourself the domain when independent domain knowledge is available. Ground yourself first, then inspect the implementation as a specimen.**
