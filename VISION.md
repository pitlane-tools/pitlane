...
**Markdown for models has no source of truth.** The site publishes a `.md` twin of every page, plus `llms.txt` and `llms-full.txt`, and derives them by parsing each prerendered `<article>` back into Markdown (`.docs/build/markdown.ts`, ~250 lines). It has to: the authored source is not what a model should read. A guide can be one `<Include>` of a shared partial, a two-mode guide carries both `<Vite>` and `<NoBuild>` sections of which a page shows one, and an install group or callout is a component call with no Markdown form until it renders. The export therefore has to express the rendered meaning, and the rendered HTML is the only place that meaning exists. Re-deriving Markdown from HTML is fragile, though: it must know every component's HTML shape, and a new component silently exports as whatever its markup happens to serialize to. The cleaner shape is for documentation components to own their Markdown rendering, with the exporter walking the source tree with variants and includes resolved. That is a decision about what a documentation component is, so it belongs with the theme above; when Pitlane packages documentation for models, the tooling for these exports (`.md` twins, `llms.txt`, per-component Markdown) is part of that package, not something each site writes.
