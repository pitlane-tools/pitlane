...
A minimal client-rendered Remix 3 app wired through the local `remix()` plugin in SPA mode (`server: false`), used two ways:
...
The automated suite restores every file it touches. If you edit files by hand, `git checkout tests/fixtures/spa-app` resets them.
