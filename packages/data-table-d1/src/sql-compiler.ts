// Vendored verbatim from @remix-run/data-table-sqlite@0.6.0
// (packages/data-table-sqlite/src/lib/sql-compiler.ts), MIT, Copyright (c)
// 2025 Shopify Inc. The full licence is in LICENSE.remix beside this package.
//
// Only the two import specifiers changed, from the private
// `@remix-run/data-table*` package names to the public `remix/*` subpaths this
// package resolves. Nothing else is edited, so a future upstream revision
// diffs cleanly against this file.
//
// It is here because `@remix-run/data-table-sqlite` exports exactly
// `createSqliteDatabase` and `SqliteDatabase` — `compileSqliteOperation` is
// internal, and its own driver reaches it through a relative import. A D1
// driver needs SQLite SQL generation without SQLite execution, so until
// upstream exposes the compiler (or ships a D1 dialect) the only options are
// to vendor this or to reimplement it. Vendoring keeps the two in step.

import type { Predicate, SqlStatement } from "remix/data-table";
import type { DataManipulationOperation } from "remix/data-table";
