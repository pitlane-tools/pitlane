...
- Raised the `remix` peer dependency to `^3.0.0-rc.1` (from `^3.0.0-beta.10`). No API changes — `createTheme`, `extend`, `select`, `<Theme />`, and the `css`/`tva`/`combine`/`cx` helpers are unchanged, and the package still has no runtime dependency on Remix beyond `createElement` from `remix/ui`.
