...
let schema = {
    ...
    color: s.color(),
    spacing: s.scale(),
    breakpoint: s.dimension(),
    container: s.dimension(),
    text: s.group(s.dimension(), { lineHeight: s.number() }),
    fontWeight: s.font.weight(),
    tracking: s.dimension(),
    leading: s.number(),
    radius: s.dimension(),
    shadow: s.shadow(),
    insetShadow: s.shadow(),
    dropShadow: s.shadow(),
    textShadow: s.shadow(),
    ease: s.easing(),
    animate: s.any(),
    blur: s.dimension(),
    perspective: s.dimension(),
    aspect: s.any(),
    default: {
        transition: {
            duration: s.duration(),
            timingFunction: s.easing(),
        },
        font: {
            family: s.font.family(),
            featureSettings: s.any(),
            variationSettings: s.any(),
        },
        mono: {
            font: {
                family: s.font.family(),
...
let tokens = {
    ...
    default: {
        ...
        font: {
            family: "--theme(--font-sans, initial)",
            featureSettings: "--theme(--font-sans--font-feature-settings, initial)",
            variationSettings: "--theme(--font-sans--font-variation-settings, initial)",
        },
        mono: {
            font: {
                family: "--theme(--font-mono, initial)",
                featureSettings: "--theme(--font-mono--font-feature-settings, initial)",
                variationSettings: "--theme(--font-mono--font-variation-settings, initial)",
            },
        },
    },
    maxWidth: {
...
type DefaultThemeInit = {
    schema: Omit<typeof schema, "text"> & {
        text: {
            readonly [SELF]: TokenSchema<"dimension">;
            lineHeight: TokenSchema<"number">;
        };
    };
    tokens: typeof tokens;
};

let theme = createTheme({ schema, tokens });
 ...
 * double-hyphen modifier spelling.
 *
 * The animation tokens name Tailwind keyframes but do not emit them.
 * An application that uses `t.animate.spin`, `t.animate.ping`,
 * `t.animate.pulse`, or `t.animate.bounce` must define the matching
 * `@keyframes` rule itself.
 *
 * @example
 * ```ts
 ...
 *
 * let theme = createTheme(DefaultTheme).select(base => ({
 *     schema: { color: s.color(), spacing: s.scale() },
 *     tokens: {
 *         color: { red: { 500: base.color.red[500] } },
 *         spacing: base.spacing.token,
 *     },
 * }));
 *
 * theme.token.spacing(4);
 * ```
 */
export let DefaultTheme: ThemeComponent<DefaultThemeInit> = theme.Theme;
